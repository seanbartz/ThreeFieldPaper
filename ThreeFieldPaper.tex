...
\label{twofield}
\ee
Here, the dot represents differentiation with respect to the $z$ coordinate.
...
Inserting this IR behavior for the dilaton and chiral field into (\ref{twofield}), we find that the chiral field behaves as
\be
\chi(z\rightarrow \infty)=6^{3/4}\sqrt{\lambda}z \, ,
\ee
linking the IR behavior of the chiral condensate with the parameter that determines the radial Regge trajectories.
However, the IR behavior of the chiral field determines the constant mass splitting between the axial vector and vector mesons for large radial quantum numbers $n$ \cite{gherghetta-kelley},
\be
\Delta m^2 \equiv \left(m_{A_n}^2-m_{V_n}^2\right)_{n\rightarrow \infty} = \frac{g_5^2}{z^2} \chi^2(z\rightarrow \infty) = g_5^2 6^{3/2} \lambda \, .
\label{eq:massgap}
\ee
Thus, there is no independent parameter in the model that controls this mass splitting.
Using the value of $\lambda = 0.1831/2$ GeV$^2$ found in the analysis \cite{gherghetta-kelley} (our definition of $\lambda$ differs from theirs by a factor of 2), we find a value for $\Delta m^2 = 53 $ GeV$^2$, which is much larger than the experimental value of 1.42 GeV$^2$.
Because this problem arises in the equation that is independent of the potential, it cannot be resolved by the choice of potential in models that do not consider the glueball condensate. 
...
The switching functions are not unique, but do allow for an accurate solution to the differential equations, as explained below.

The potential $U$, because it is a function of the background fields, will change depending upon the behavior of the fields.
In particular, the additional $\Delta U$ term is numerically determined such that (\ref{U}-\ref{phi}) are solved by the background fields.
A numerical nonlinear optimization routine, namely the interior point algorithm \cite{interior}, is used to select the parameters that minimize the error in the finite-difference approximations to equations ($\ref{C}), (\ref{chi}),$ and ($\ref{G}$), thus ensuring that the parameterized fields are solutions to the background fields, within numerical tolerances.
In this analysis, the fields $\phi$, $\chi$, and $G$ solve the equations (\ref{C}-\ref{G}) to an accuracy of one part in $10^4$. 
...
In addition to obtaining solutions that solve the background equations, we also wish to achieve the best possible global visual fit to the vector and axial-vector meson spectra.   
...
\ee
These equations are analytically solvable in the IR limit\footnotemark, but full analysis requires the use of a numerical shooting method to find the mass eigenvalues.
...
\footnotetext{The analytical solution gives a large-$n$ spectrum with a slope of 8$\lambda$, differing from the slope of 4$\lambda$ found in other AdS/QCD models.
This difference arises because of the $\exp(-2\Phi)$ pre-factor in the action (\ref{eqStringAction}) compared to the pre-factor of $\exp(-\Phi)$ used in other models \cite{karch-katz-son-adsqcd, gherghetta-kelley}. 
This factor of two is compensated by the numerical value of $\lambda$ shown in Table \ref{tabParam}, which is half the value used in other models.
Thus, the notational choice does not affect the resulting meson spectra.}
...
\bibitem{karch-katz-son-adsqcd} A. Karch, E. Katz, D.T. Son, and M.A. Stephanov, Phys. Rev. D {\bf 74}, 015005 (2006).

\bibitem{gherghetta-kelley} T. Gherghetta, J. I. Kapusta, and T. M. Kelley, Phys. Rev. D {\bf 79}, 076003 (2009).

\bibitem{interior} R. Byrd, J. Gilbert, J. Nocedal, Mathematical Programming {\bf 89}, 149 (2000).
...
\bibitem{Herzog2007} C.P. Herzog, Phys. Rev. Lett. {\bf 98}, 091601 (2007).

\bibitem{Bayona2008} C.A. Ballon Bayona, H. Boschi-Filho, N.R.F. Braga, and Leopoldo A. Pando Zayas, Phys. Rev. D {\bf 77}, 046002 (2008).
...
\bibitem{Bugg} D. Bugg, Physics Reports {\bf 397}, 257 (2004).

\bibitem{Bertin} A. Bertin \emph{et al.} [OBELIX Colaboration], Phys. Lett. B {\bf 414}, 220 (1997).

\bibitem{PDG} J. Beringer {\emph et al.} (Particle Data Group), Phys. Rev. D {\bf 86}, 010001 (2012).
